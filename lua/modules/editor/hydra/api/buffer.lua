...
-- Try fn(), on failure run fallback() if provided
...
local function safe_cmd(command, fallback_cmd)
    return function()
        local ok = pcall(vim.cmd, command)
        if not ok and fallback_cmd then
...
local function buffer_move()
    vim.ui.input({ prompt = "Move buffer to:" }, function(idx)
        ...
        if idx then
            local ok = pcall(function() require("three").move_buffer(idx) end)
            if not ok then vim.notify("three.nvim not available", vim.log.levels.WARN) end
        end
    end)
end
local cmd = vim.cmd
...
local bufferline_commands = {
    ...
    J = {
        safe(function() require("three").next() end, function() vim.cmd("bnext") end),
        { private = true, nowait = true, desc = "[G]oto next [B]uffer", exit = false },
    },
    K = {
        safe(function() require("three").prev() end, function() vim.cmd("bprev") end),
        { private = true, nowait = true, desc = "[G]oto Prev [B]uffer", exit = false },
    },

    q = {
        safe(function() require("three").smart_close() end, function() vim.cmd("bdelete") end),
        { private = true, nowait = true, desc = "[C]lose Smart", exit = true },
    },
    Q = {
        safe(function() require("three").close_buffer() end, function() vim.cmd("bdelete") end),
        { private = true, nowait = true, desc = "[B]uffer [C]lose", exit = true },
    },
    M = {
        safe(function() require("three").hide_buffer() end, function() vim.cmd("bdelete") end),
        { private = true, nowait = true, desc = "[B]uffer [H]ide", exit = true },
    },

    P = {
        safe(function() require("three").open_project() end),
...
local config = {
    Buffer = {
        ...
        m = {
            safe_cmd("ReachOpen marks", "Telescope marks"),
            { exit = true, private = true, nowait = true, desc = "Reach Marks" },
        },
        W = {
            safe_cmd("ReachOpen tabpages", "Telescope buffers"),
            { exit = true, private = true, nowait = true, desc = "Reach TabPage" },
        },
        C = {
            safe_cmd("ReachOpen colorschemes", "Telescope colorscheme"),
        ...
        b = {
            safe(
                function() require("utils.telescope").buffers() end,
                function() vim.cmd("Telescope buffers") end
            ),
