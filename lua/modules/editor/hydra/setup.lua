...
local function loadHydraModules(path, prefix)
    ...
    for _, path in ipairs(path_list) do
        ...
        when(not vim.tbl_contains(EXCLUDE_TABLE, name), function()
            ...
            if not ok then
                vim.notify("hydra: failed to load " .. name .. ": " .. tostring(err), vim.log.levels.WARN)
            end
        end)
    end
end
...
local function loadHydraAPI()
    ...
    for _, path in ipairs(api_list) do
        ...
        when(not vim.tbl_contains(exclude_table, name), function()
            ...
            if not ok or type(data) ~= "table" or not data[1] or not data[2] then
                return
            end
            local cfg = data[1][data[2]]
            if cfg and cfg.body then
                binds[name] = cfg.body
            end
            pcall(function()
