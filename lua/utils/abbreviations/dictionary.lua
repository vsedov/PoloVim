...
M.global_abbrevs = {
    ...
    cabbrevs = {
        { "<expr> Cp", [[((getcmdtype()==':'&&getcmdpos()<=3)?'C p':'Cp')]] },
        { "<expr> Cd", [[((getcmdtype()==':'&&getcmdpos()<=3)?'Cdisplay':'Cd')]] },
        { "<expr> csa", [[((getcmdtype() == ':' && getcmdpos() <= 4)? 'cs add'  : 'csa')]] },
        { "<expr> csf", [[((getcmdtype() == ':' && getcmdpos() <= 4)? 'cs find' : 'csf')]] },
        { "<expr> csk", [[((getcmdtype() == ':' && getcmdpos() <= 4)? 'cs kill' : 'csk')]] },
        { "<expr> csr", [[((getcmdtype() == ':' && getcmdpos() <= 4)? 'cs reset' : 'csr')]] },
        { "<expr> css", [[((getcmdtype() == ':' && getcmdpos() <= 5)? 'cs show' : 'css')]] },
        { "<expr> csh", [[((getcmdtype() == ':' && getcmdpos() <= 4)? 'cs help' : 'csh')]] },
        { "<expr> W", [[((getcmdtype() is# ':' && getcmdline() is#'W')?('w'):('W'))]] },
        { "<expr> Q", [[((getcmdtype() is# ':' && getcmdline() is#'Q')?('q'):('Q'))]] },
        { "syn", [[set syntax=]] },
        { "pc", "PackerCompile<CR>" },
        { "pu", "PackerUpdate<CR>" },
        { "ps", "PackerSync<CR>" },
        { "lcc", "LuaCacheClear<CR>" },
        { "W!", [[w!]] },
        { "Q!", [[q!]] },
        { "Qall!", [[qall!]] },
        { "Wq", [[wq]] },
